...
export default function Summary() {
  ...
  const getTable = async () => {
    ...
    if(startDate && endDate && (new Date(endDate) - new Date(startDate) >= 0)){
      ...
      if(selectedCoupon == "general"){
        const table = []
        const csvL = [["Date"],["Total"]]
        for(let i = 0;i < sorted.length;i++){
          let num = sorted[i].data.length
          const sp = sorted[i].date.split("-")
          while(sorted[i].date != iDate.toISOString().split("T")[0]){
            const idsp = iDate.toISOString().split("T")[0].split("-")
            table.push({date: idsp[1]+"-"+idsp[2], s0: 0, fullDate: iDate.toISOString().split("T")[0]})
            csvL[0].push(sp[1]+"-"+sp[2])
            csvL[1].push(0)
            iDate.setDate(iDate.getDate() + 1)
          }
          table.push({date: sp[1]+"-"+sp[2], s0: num, fullDate: sp[0]+"-"+sp[1]+"-"+sp[2]})
          csvL[0].push(sp[1]+"-"+sp[2])
          csvL[1].push(num)
          iDate.setDate(iDate.getDate() + 1)
        }
        for(let d = iDate; d <= new Date(endDate); d.setDate(d.getDate() + 1)) {
          ...
          table.push({date: idsp[1]+"-"+idsp[2], s0: 0, fullDate: d.toISOString().split("T")[0]})
          csvL[0].push(idsp[1]+"-"+idsp[2])
          csvL[1].push(0)
        }
        setTbData(table)
        setCsvData(csvL)
  ...
  return (
    <div className="relative px-[5%] pb-12">
      <div className="gap-4 flex flex-row content-start items-center mt-6">
        ...
        <select className="text-gray-600 bg-white p-2 h-14 w-24 rounded-md text-center" value={selectedCoupon} onChange={handleChange}>
      ...
      {selectedCoupon=="general" ? <>
        <div className="flex mt-5 flex-col h-screen">
          <h1 className="font-semibold text-2xl">Data</h1>
          <Table data={tbData} selectedCoupon={selectedCoupon} dep="all"/>
        </div></> :
      <>
        <div className="flex mt-5 flex-col">
          <h1 className="font-semibold text-2xl">Dana Ladkrabang</h1>
          <TableShift data={tbData} selectedCoupon={selectedCoupon} dep="1"/>
        </div>
        <div className="flex mt-5 flex-col">
          <h1 className="font-semibold text-2xl">GXO</h1>
          <TableShift data={tbGxoData} selectedCoupon={selectedCoupon} dep="2"/>
        </div>
        <div className="flex mt-5 flex-col">
          <h1 className="font-semibold text-2xl">Other</h1>
          <TableShift data={tbOtherData} selectedCoupon={selectedCoupon} dep="3"/>
        </div>
      </>}
    </div>
  );
...
const Table = ({data, selectedCoupon, dep}) => {
  ...
  const pushToGeneralLogs = ({date}) => {
    navigate("/generallogs?date=" + date + "&type=" + selectedCoupon + "&dep=" + dep)
  }
    ...
    <table className="w-full">
      <tbody>
        <tr className="summary-tr">
          <th className="text-lg summary-td">Date</th>
          {data.map((item => {
            return(
              <th key={item.date}>
                {item.date}
        ...
        <tr className="summary-tr">
          <td className="text-lg summary-td font-bold">Total</td>
          {data.map((item => {
            return(
              <td key={"1"+item.date} onClick={() => pushToGeneralLogs({date:item.fullDate})}>
                {item.s0}
              </td>
            )
          }))}
...
const TableShift = ({data, selectedCoupon, dep}) => {
  const navigate = useNavigate()
  const pushToLogs = ({date, shift}) => {
    ...
    <table className="w-full">
      <tbody>
        ...
        <tr className="summary-tr">
          ...
          {data.map((item => {
            return(
              <td key={"1"+item.date} onClick={() => pushToLogs({date:item.fullDate, shift: "1"})}>
                {item.s1}
              </td>
            )
          }))}
        ...
        <tr className="summary-tr">
          ...
          {data.map((item => {
            return(
              <td key={"2"+item.date} onClick={() => pushToLogs({date:item.fullDate, shift: "2"})}>
                {item.s2}
              </td>
            )
          }))}
        ...
        <tr className="summary-tr">
          ...
          {data.map((item => {
            return(
              <td key={"3"+item.date} onClick={() => pushToLogs({date:item.fullDate, shift: "3"})}>
                {item.s3}
              </td>
            )
          }))}
        ...
        <tr className="summary-tr">
          ...
          {data.map((item => {
            return(
              <td key={"4"+item.date} onClick={() => pushToLogs({date:item.fullDate, shift: "4"})}>
                {item.s4}
              </td>
            )
          }))}
        ...
        <tr className="summary-tr">
          ...
          {data.map((item => {
            return(
              <td key={"0"+item.date} onClick={() => pushToLogs({date:item.fullDate, shift: "0"})}>
                {item.s0}
              </td>
            )
          }))}
